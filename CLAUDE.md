...
**Locked slot → flexibility screen → partner picker flow**: Clicking a locked slot calls `handleLockedSlotClick` (Hour View) or `handleCourtViewLockedSlotClick` (Court View), which fetches the player list (see below), then shows a **Booking Preferences** screen before the partner picker. The flexibility screen lets the user pick a court type preference (gated/edge/any) and check backup alternative slots (nearby times at the same club within ±2 hours, or the same time at other clubs). `gatherAlternativeSlots(slotInfo, lastFetchState)` (IIFE-scope pure function) extracts these from the last availability fetch, using `BOOKING_WINDOW_DAYS` (3) to tag each as locked or available. `buildFlexibilityScreenHtml()` renders the screen; `bindFlexibilityScreenInteractions()` wires pill selection, checkbox feedback, and Next/Back/Cancel buttons. On Next, `collectFlexibilityPrefs()` returns `{ courtTypePref, alternativeSlots }`, which is attached to `slotInfo.flexibilityPrefs` before `injectPartnerPickerIntoHosts()` renders the partner picker. `injectPartnerPickerIntoHosts()` is an extracted helper shared by both Hour View and Court View flows to avoid duplicating photo-fetch and panel-injection logic. No Angular state machine interaction occurs. The user selects partners and clicks Schedule; `getScheduledBookingService().scheduleBooking()` persists the record (including `alternativeSlots` and `courtTypePref`-sorted `fallbackCourts`) and sets timers.
...
- **Scheduled bookings**: Locked slots (beyond the 3-day booking window) are clickable. Clicking one opens a **Booking Preferences** screen (court type preference pills, alternative time/club backup slots), then a partner picker — both built entirely by the helper with no Angular state machine involvement. After the user selects partners and taps Schedule, the helper persists the booking (including `alternativeSlots` and `courtTypePref`) and the Worker fires the two-step booking API at the exact moment the window opens, trying the primary court, fallback courts (sorted by type preference), then alternative slots in order. A "Pending Bookings" section on `/bookings` shows countdown, scheduled time, and a Cancel button for each pending booking. Failed attempts show with a red error row and a Dismiss button.
