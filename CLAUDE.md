...
**Locked slot → flexibility screen → partner picker flow**: Clicking a locked slot calls `handleLockedSlotClick` (Hour View) or `handleCourtViewLockedSlotClick` (Court View), which fetches the player list (see below), then shows a **Booking Preferences** screen before the partner picker. The screen is split into two sections:

- **"Book a different slot now"**: Slots within the 3-day booking window. Radio selection — picking one switches the Next button to "Book now" and disables the schedule section. On Next, `slotInfo` is updated to the chosen slot (including `allCourts` from the gathered alternatives) and booking proceeds immediately (fireAtMs computes to past, so the Worker fires on the next cron tick).
- **"Add backup slots"**: Locked slots beyond the window. Checkboxes add slots to a numbered priority list with drag handles for reordering (desktop drag-and-drop + mobile touch). The priority list uses the same drag pattern as `getClubOrderWidgetController()`. A persistent note explains: *"For each slot, the system tries gated courts first, then edge, then any available."*

Both sections show court names with type badges (`[G]`/`[E]`/`[H]`) via `formatCourtList()`. A time range stepper (two inline displays with ▲/▼ buttons, 30-minute steps, clamped 5 AM–11 PM) lets users widen or narrow the nearby-time window beyond the ±2h default; changing it re-filters the slot lists via `rerenderSlots()`. `gatherAlternativeSlots(slotInfo, lastFetchState, rangeMinutes)` (IIFE-scope pure function) extracts alternatives from the last availability fetch. `DEFAULT_ALT_RANGE_MINUTES` (120) is the default. Each court is annotated with `courtType` via `classifyCourtType()`. `BOOKING_WINDOW_DAYS` (3) tags slots as locked or available.

`buildFlexibilityScreenHtml()` renders the screen; `bindFlexibilityScreenInteractions(flexScreen, slotInfo, lastFetchState, onNext, onBookNow, onBack)` wires radio/checkbox/drag/stepper interactions. `collectFlexibilityPrefs()` returns `{ bookNowSlot, alternativeSlots }` — the `bookNowSlot` path calls `onBookNow`, the schedule path calls `onNext`. `injectPartnerPickerIntoHosts()` is an extracted helper shared by both Hour View and Court View flows. When `courtId` is null (book-now path), `scheduleBooking` auto-selects the best court from `allCourts` (sorted gated > edge > standard).
