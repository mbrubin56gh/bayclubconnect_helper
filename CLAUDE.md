...
**Club navigation strip** — `injectClubNavStrip()` (called from `tagColumns()`, not `install()`, so columns are guaranteed tagged) injects a row of club shortcut buttons as a sibling immediately before `app-booking-calendar`. Clicking a button scrolls to that club's first court column. Key implementation details: (1) Angular uses a separate `div.floating-scroll` element as the scrollbar driver — it listens for `scroll` events on that element and mirrors its `scrollLeft` to the content columns. Scrolling `.booking-calendar-columns-floating-scroll` directly bypasses this sync and leaves the scrollbar stuck. Always scroll `div.floating-scroll` instead. (2) Use `getBoundingClientRect` diff (not `offsetLeft`) to compute the target `scrollLeft` since `offsetLeft` is relative to `offsetParent`, which may not be the scroll container when `flex-row-reverse` is in play. (3) Call `btn.blur()` after scrolling so the button loses `:focus` styling and keyboard/scroll events return to the document. The strip is idempotent (injected once per session) and cleaned up in `clear()`.
