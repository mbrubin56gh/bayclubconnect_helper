...
**`getMergedCourtsOrder()`** on `getBookingStateService()` stores the `[{courtId, clubId, courtName, courtOrder, openingHours}]` array built during courts merging. `openingHours` is the raw per-day array from the `/courts` API response (`[{ dayOfWeek, openingHours: [{ fromInMinutes, toInMinutes }] }]`). `getCourtOpenHoursForDate(courtEntry, dateStr)` resolves it to a single `{ fromInMinutes, toInMinutes }` for a given date, used to exclude courts-closed slots from hover highlighting and straddle detection. `getNativeCourtColumnsService` uses this to tag each rendered `app-booking-calendar-column` with `data-bc-club-id` and `data-bc-court-id` by index position, since Angular renders columns in the same order courts appear in the response.
...
**Weather strip** — `injectWeatherStrip()` (called from `tagColumns()` on every reconcile) appends a `div[data-bc-weather-strip]` to `app-booking-calendar` (`position:relative` via CSS). The strip is a narrow (`20px`) absolutely-positioned overlay aligned with the left edge of Angular's time axis column. For each on-the-hour slot from 5 AM through 10 PM, an emoji element is absolutely positioned at the matching vertical offset (`gridTop + slotIndex × 64 + 32`). Rain emojis include a rain-probability `%` child div. The strip is idempotent: it stamps `data-bc-weather-date` with the booking date and returns early if a strip for that date already exists — this breaks the MutationObserver re-entry loop that would otherwise occur when `cal.appendChild(strip)` fires `tagColumns()` again. CSS also adds `padding-left:20px; box-sizing:border-box` to `div.booking-calendar-time-slot` (to indent time text into the strip's space) and `font-size:10px; white-space:nowrap` to the span (to prevent "5:00 AM" wrapping in the narrowed column). Rain percentage divs are children of the emoji element inside the strip, so they cannot affect the time label layout.

**Out-of-hours slot dimming** — `tagOneColumn` stamps `data-bc-closed` on every slot outside the court's operating hours (derived from `getCourtOpenHoursForDate(courtEntry, viewDate)`). `injectColumnColorStyles()` injects a CSS rule that dims these slots to 25% opacity with `pointer-events:none`. As belt-and-suspenders, `onCalendarSlotClick` and `onCalendarMouseOver` also check `hasAttribute('data-bc-closed')` as an early return guard — this prevents interaction even if CSS `pointer-events:none` is bypassed by capture-phase handlers. The hover highlight window-fill algorithm (phases 1 and 2) also stops at `data-bc-closed` slots so the highlight never extends into hours before or after the court opens.

**Courtsheet state synchronization on date change** — `getBookingStateService().updateCourtsheetState(date, courtsheetEventsByClubId)` patches `lastFetchState` in place without replacing the whole object. The `/courts` intercept calls it with just the date (events param omitted) immediately before dispatching the merged response to Angular, so `tagColumns` (fired by MutationObserver) already has the correct date for operating-hours dimming. The `/courtsheet` events intercept calls it with both date and a fresh `courtsheetEventsByClubId` map after merging. Without this, `lastFetchState.params.date` would remain stale from the original availability fetch, causing `getCourtOpenHoursForDate` to compute hours for the wrong day-of-week on date change.

**Sticky column header** — `injectStickyColumnHeader()` (called from `tagColumns()`) creates a `position:fixed` bar (`data-bc-sticky-header`) at the top of the viewport that mirrors the column names with club colors. The bar appears when the native `div.booking-calendar-column-header` elements scroll above the viewport (checked via `getBoundingClientRect().bottom < 0`). Label widths are synced to actual column widths via `syncWidths()`. Horizontal scroll is synced with the floating-scroll driver (and the content container for mobile) via `translateX`. The bar is injected once per Court View session (idempotent guard) and cleaned up in `clear()` via a `stickyHeaderCleanup` closure that removes event listeners and the bar element.
...
**Locked slots in Court View** — Slots with `booking-calendar-column-time-slot-unavailable` are outside the 3-day booking window and inert in Angular's native UI. We intercept clicks in capture phase (before Angular's bubble-phase listener) and call `stopPropagation()` to suppress Angular's error, then open the same partner picker / schedule panel used by Hour View locked slots (`handleCourtViewLockedSlotClick`). The function accepts optional `fireAtFromMinutesOverride` (for straddle fire-time) and `toMinutesOverride` (for straddle "Book now" partial duration). The panel is injected as a `position:fixed` overlay on `document.body` (attribute `data-bc-cv-schedule-overlay`) rather than into `getTimeSlotHosts()`, because those hosts contain no `app-court-time-slot-item` elements in Court View and would return empty. Hiding `app-booking-calendar` is deliberately avoided — it would trigger the MutationObserver reconcile and immediately clear the overlay. Slot time is derived from `COURT_VIEW_GRID_START_MINUTES` (5 AM = 300 min) plus the slot's index within the column × 30 min. Slots occupied by existing events (checked against `courtsheetEventsByClubId`) silently ignore clicks.

**Locked slot hover highlighting** — `injectColumnColorStyles()` also injects CSS rules that give `cursor:pointer` to all locked slots in tagged columns, and apply a teal inset box-shadow highlight to elements tagged `data-bc-hover-highlight`. `onCalendarMouseOver` (registered in **capture** phase on `app-booking-calendar`) detects the hovered locked slot, computes a duration-aware highlight window, and sets the attribute on each slot in the window with a position value (`first`/`middle`/`last`/`only`). The CSS uses the position value to add top/bottom inset shadows only on the outer slots, producing a single unified rectangle with no interior horizontal dividers. Window algorithm: extend downward (later in time) from the hover slot until the full booking duration is covered, a booked event is hit, or a `data-bc-closed` slot is hit; then fill any shortfall by extending upward (earlier in time) from the hover slot, stopping at the next booked event, closed slot, or column boundary. Slots with `data-bc-closed` are skipped early before the window algorithm runs. `clearLockedSlotHover()` is called on `mouseleave` and in `clear()`.

**Straddle detection and prompt** — `computeStraddleInfo(column, slotIndex, slotDate, lastFetchState)` detects when a hovered available slot's full booking duration crosses the booking window boundary (some sub-slots available, later ones locked). It scans **forward** from `slotIndex` (the hover point) for window-locked sub-slots: unavailable in Angular's DOM, not covered by a courtsheet event, within the court's operating hours (from `openingHours` in `mergedCourtsOrder`), **and** with a Pacific slot time more than 3 days from now. Both the operating-hours check and the 3-day time check are essential: courts-closed slots (e.g. 10pm–11pm) are outside operating hours and never bookable; the time check handles the same slots on dates within the 3-day window where the operating-hours check alone is insufficient. Returns `{ fromMinutes, partialToMinutes, fullToMinutes, fireAtFromMinutes, firstLockedIndex, lastLockedIndex, durationSlots }` or `null` if no straddle exists. When a straddle is found in `onCalendarMouseOver`, `stopPropagation()` blocks Angular's click and our highlight spans both the available portion and the locked extension. When a straddle is found in `onCalendarSlotClick`, `stopPropagation()` blocks Angular's click — Angular rejects straddle clicks because the full duration extends into locked territory, so its state machine never advances. On mobile (touch devices), there is no preceding `mouseover` event, so `onCalendarSlotClick` also applies the straddle highlight (`data-bc-straddle-active` + `data-bc-hover-highlight` on each slot) before showing the prompt, giving the user visual context behind the semi-transparent overlay. `showStraddlePrompt()` injects a `position:fixed` modal offering: (1) "Book now" — delegates to `handleCourtViewLockedSlotClick(slot, undefined, partialToMinutes)` which opens the partner picker with the truncated duration; the slot is within the booking window so the Worker fires immediately (next cron tick); (2) "Schedule" — delegates to `handleCourtViewLockedSlotClick(slot, fireAtFromMinutes)` passing the override so the Worker fires when the last locked sub-slot opens. `slotInfo.fireAtFromMinutes` flows through `scheduleBooking()` to `computeFireAtMs` for correct fire time. Both paths and the Cancel/backdrop dismiss call `dismissAndClearNativeSelection()` which removes Angular's add-tile and hides the bottom bar. The prompt's `dismiss()` calls `clearLockedSlotHover()` to clean up highlight attributes on both mobile and desktop paths.

**Angular hover tile suppression** — Angular's hover highlight is not CSS `:hover` — it injects a child `div.booking-calendar-add-tile` (sized to the full booking duration in pixels) into the hovered slot via zone.js change detection. JS event interception cannot block this from a Tampermonkey isolated world because `stopImmediatePropagation()` only stops listeners within the same world's event queue. Instead, `injectColumnColorStyles()` injects a CSS rule `app-booking-calendar-column[data-bc-straddle-active] .booking-calendar-add-tile { display:none !important; }` that hides the tile whenever our straddle attribute is active, leaving our own `data-bc-hover-highlight` styling as the only visible highlight.

**Edge/gated/hitting-wall badges** — `tagColumns()` stamps E/G/H badges on each column's `div.court-name` element (not the column element itself) via `data-bc-badges`. The CSS rule `div.court-name[data-bc-badges]::after { content: "  " attr(data-bc-badges); }` appends the badge string after the court name. The attribute must live on `div.court-name` directly because CSS `attr()` in `::after` resolves on the element the pseudo-element belongs to, not on any ancestor. Angular abbreviates court names in Court View ("PB1", "2", bare "Pickleball"); `tagOneColumn()` uses the API `courtName` from `mergedCourtsOrder` as the canonical source of truth and overwrites the DOM text so columns display full names ("Pickleball 1"). Redwood Shores uses `['*']` in `EDGE_COURTS` (matched by `isCourtEdge` before the name check) because all RS courts are edge courts and Angular renders their names inconsistently. `injectBadgeLegend()` inserts a static legend bar as a sibling immediately before `app-booking-calendar`. The legend and all `data-bc-badges` attributes are cleaned up in `clear()`. The courtsheet `/courts` intercept now calls `setMergedCourtsOrder()` after merging, stamping `bc_clubId` on every court item so `tagColumns()` has club mapping data when entering Court View without going through Hour View first.

**Firefox Android dual-calendar** — Angular on Firefox Android keeps two `app-court-time-slot-select` elements (and therefore two `app-booking-calendar` elements) after the slide-in animation. The old view stays permanently at `translateX(590px)` off-screen; the visible one has no transform. `querySelector('app-booking-calendar')` returns the off-screen copy. `findVisibleCalendar()` (inside `getNativeCourtColumnsService`) walks ancestors of each calendar checking `getComputedStyle().transform` and returns the one without a transformed ancestor. All Court View code that needs the on-screen calendar (`install()`, `injectClubNavStrip()`, `injectWeatherStrip()`, scroll handlers) must use `findVisibleCalendar()` instead of `querySelector`. `tagColumns()` iterates each `app-booking-calendar` independently (per-calendar indexing) so both copies get `data-bc-club-id`, badges, and pod-conflict attributes — CSS attribute selectors then work regardless of which copy Angular is rendering. On Chrome (single calendar, no lingering transform) `findVisibleCalendar()` returns the only element, so the extra walk is a no-op.

**Club navigation strip** — `injectClubNavStrip()` (called from `tagColumns()`, not `install()`, so columns are guaranteed tagged) injects a row of club shortcut buttons as a sibling immediately before `app-booking-calendar` (using `findVisibleCalendar()`). Clicking a button scrolls to that club's first court column. Key implementation details: (1) Angular uses a separate `div.floating-scroll` element as the scrollbar driver — it listens for `scroll` events on that element and mirrors its `scrollLeft` to the content columns. Scrolling `.booking-calendar-columns-floating-scroll` directly bypasses this sync and leaves the scrollbar stuck. Always scroll `div.floating-scroll` instead. (2) On Firefox Android mobile, `div.floating-scroll` does not exist; the scroll handler walks up from the target column to find the nearest horizontally scrollable ancestor. (3) On Firefox Android, tagged columns live in the off-screen calendar; the scroll handler maps the target column's index to the corresponding column in the visible calendar via `findVisibleCalendar()`. (4) Use `getBoundingClientRect` diff (not `offsetLeft`) to compute the target `scrollLeft` since `offsetLeft` is relative to `offsetParent`, which may not be the scroll container when `flex-row-reverse` is in play. The reference rect must come from the content container (`.booking-calendar-columns-floating-scroll`), not `div.floating-scroll` — the two elements can have different bounding rects, and using the scroll driver's rect causes the scroll to stop short. (5) Call `btn.blur()` after scrolling so the button loses `:focus` styling and keyboard/scroll events return to the document. The strip is idempotent (injected once per session) and cleaned up in `clear()`.
...
**Locked slot → flexibility screen → partner picker flow**: Clicking a locked slot calls `handleLockedSlotClick` (Hour View) or `handleCourtViewLockedSlotClick` (Court View), which fetches the player list (see below), then shows a **Scheduled Booking Preferences** screen before the partner picker. The screen auto-scrolls to the top on display (Hour View) and is split into two sections:

- **"Book a different slot now"**: Slots within the 3-day booking window. Mutual-exclusion checkboxes — checking one unchecks others and disables the schedule section. Multi-court slots render as expandable cards: clicking the card checks it, and expanding reveals individual court options sorted best-first (gated > edge > hitting wall > standard) so the user can pick a specific court. If the user checks a card without choosing a court, expanding it auto-highlights the best default. `data-selected-court-id` / `data-selected-court-name` on the card track the explicit choice. On Next, `slotInfo` is updated to the chosen slot (including `allCourts` and optionally `courtId`/`courtName`) and booking proceeds immediately (fireAtMs computes to past, so the Worker fires on the next cron tick).
- **"Add backup slots"**: Locked slots beyond the window. Checkboxes add slots to a numbered priority list with drag handles for reordering (desktop drag-and-drop + mobile touch). The priority list uses the same drag pattern as `getClubOrderWidgetController()`. A persistent note explains: *"For each slot, the system tries all available courts — gated first, then edge, then any available."*

Both sections show court names with colored type badges (gold `G`/`E`/`H` via HTML spans) via `formatCourtList()`. Club names use per-club accent colors from `CLUB_COLUMN_COLORS` in both the flexibility screen and the schedule panel. A time range stepper (two inline displays with ▲/▼ buttons, 30-minute steps, clamped 5 AM–11 PM) lets users widen or narrow the nearby-time window beyond the ±2h default; changing it re-filters the slot lists via `rerenderSlots()`. `gatherAlternativeSlots(slotInfo, lastFetchState, rangeMinutes)` (IIFE-scope pure function) extracts alternatives from the last availability fetch. `DEFAULT_ALT_RANGE_MINUTES` (120) is the default. Each court is annotated with `courtType` via `classifyCourtType()`. `BOOKING_WINDOW_DAYS` (3) tags slots as locked or available.

A fixed white bottom bar (`data-bc-flex-bottom-bar`) with Cancel and Next buttons stays visible regardless of scroll. The bar's inner container (`data-bc-flex-bottom-inner`) is dynamically sized to match the flex screen's bounding rect so the Next button's right edge aligns with the slot cards. The bottom bar is a sibling of the flex screen (not nested inside it), injected once into `document.body` for Hour View or into the Court View overlay. All cleanup paths (`removeFlexScreens`, `removeCvOverlays`) remove both `[data-bc-flex-screen]` and `[data-bc-flex-bottom-bar]`.

`buildFlexibilityScreenHtml()` renders the screen; `bindFlexibilityScreenInteractions(flexScreen, slotInfo, lastFetchState, onNext, onBookNow, onBack)` wires checkbox/expand/drag/stepper interactions. `collectFlexibilityPrefs()` returns `{ bookNowSlot, alternativeSlots }` — the `bookNowSlot` includes `courtId`/`courtName` when the user explicitly picked a court. The `bookNowSlot` path calls `onBookNow`, the schedule path calls `onNext`. `injectPartnerPickerIntoHosts()` is an extracted helper shared by both Hour View and Court View flows. When `courtId` is null (book-now path without explicit court choice), `scheduleBooking` auto-selects the best court from `allCourts` (sorted gated > edge > standard).
...
**Fire sequence**: Handled entirely server-side by the Cloudflare Worker cron (see below). The browser tab does not need to stay open. The Worker tries: (1) primary court, (2) fallback courts in order (sorted gated > edge > standard), (3) `alternativeSlots` — each alt slot triggers a fresh availability fetch via `fetchAvailabilitySnapshot` and tries every available court at that time/club. On alt-slot success, the result includes `usedAlternativeSlot`, `alternativeClubName`, `alternativeFromMinutes`, `alternativeToMinutes`. On success or failure, the Worker sends an email notification via Resend from `notifications@bayclubhelper.app` — both to the scheduler (`notificationEmail`) and to each partner whose email was captured at scheduling time (`partnerEmails`). When an alternative slot was used, the email includes a substitution note with the actual club and time. Partner emails are looked up from the cached player list at the moment `scheduleBooking` is called; partners without a stored email are silently skipped. The scheduler's email is not sent a second time if it also appears in `partnerEmails`. Partner notification subjects use `booking.userName` as the scheduler label, falling back to `booking.notificationEmail`, then a generic phrase.

**Court type tagging**: At scheduling time, `classifyCourtType(clubId, courtName)` tags each court as `'gated'`, `'hitting_wall'`, `'edge'`, or `'standard'`. The primary court's type is stored as `primaryCourtType` on the booking record; each fallback court carries a `courtType` field. The Worker's slot availability check uses these tags to prefer same-type fallbacks when auto-switching. `SLOT_CHECK_STATUS` enum values: `UNKNOWN` (initial), `AVAILABLE` (primary court still open), `TAKEN` (primary taken, no fallbacks), `SWITCHED` (auto-switched to fallback). The `/bookings` pending section shows a warning for `TAKEN` and an informational "Switched from X" message for `SWITCHED`.
...
- **Scheduled bookings**: Locked slots (beyond the 3-day booking window) are clickable. Clicking one opens a **Booking Preferences** screen with two sections: "Book a different slot now" (available slots, radio selection for immediate booking) and "Add backup slots" (locked slots, checkboxes with drag-to-reorder priority list). Court names with type badges are shown. A time range stepper adjusts the nearby-time window. Then a partner picker — both built entirely by the helper with no Angular state machine involvement. After the user selects partners and taps Schedule (or Book now), the helper persists the booking and the Worker fires the two-step booking API, trying the primary court, then all fallback courts (gated > edge > any available), then alternative slots in priority order. A "Pending Bookings" section on `/bookings` shows countdown, scheduled time, and a Cancel button for each pending booking. Failed attempts show with a red error row and a Dismiss button.
...
- **D1 database**: `bayclubconnect-history` (id `e1f2166f-1c61-47f4-8675-bfa4a003d29a`); bound as `DB`; tables: `booking_history` (permanent record of completed/failed/cancelled bookings, written via `appendToHistory()` with `INSERT OR IGNORE`), `availability_snapshots` (ambient availability data recorded during slot check cron — `fetchAvailabilitySnapshot()` returns rich metadata, `recordAvailabilitySnapshot()` INSERTs to D1; zero extra API calls).
- **Cron (per-minute)**: finds `status === 'pending'` bookings whose `fireAtMs` has passed, marks `firing`, tries primary court → fallback courts → `alternativeSlots` (each alt fetches fresh availability via `fetchAvailabilitySnapshot`), saves result (including `usedAlternativeSlot` / `alternativeClubName` / `alternativeFromMinutes` / `alternativeToMinutes` when an alt succeeds), sends email with substitution note when applicable
- **Cron (slot check, every 6 hours)**: `runSlotCheckTick` checks availability for pending bookings whose `fireAtMs` is still in the future. Groups by user (one token rotation per user per cycle). If the primary court is taken, auto-switches to the best available fallback (preferring same `courtType` as `primaryCourtType`), updates `bookingBody.courtId`, `originalCourtName`, `slotLabel`, `fallbackCourts`, sets `slotCheckStatus` to `'switched'`, and sends a notification email. If no fallbacks are available, sets `slotCheckStatus` to `'taken'` and sends a warning email. Skips bookings checked within the last 5 hours (`lastSlotCheckMs`). `slotCheckNotifiedAt` prevents duplicate notification emails.
...
- **HTTP endpoints**:
  ...
  - `GET /availability-history` — last 200 availability snapshots from D1 as JSON (secret required; header or `?secret=`)
...
- Straddle prompt tests navigate to Court View on date+3 and dynamically find the available→locked slot boundary.  They skip gracefully when the 3-day window boundary falls outside court hours (e.g. very early or late Pacific time) and no straddle-eligible slot exists.
