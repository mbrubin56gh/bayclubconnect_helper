...
(function () {
    ...
    // This service injects all clubs' court data into Angular's native availability
    // response so Angular renders every court as a native column.  We then tag each
    // rendered column with data-bc-club-id and show/hide by selected club.  Because
    // users click real Angular-rendered slots, the Next button routing works without
    // any state machine hacking.
        ...
        return function getNativeCourtColumnsService() {
            ...
            function findVisibleCalendar() {
                ...
                for (var i = 0; i < all.length; i++) {
                    ...
                    var ancestor = all[i].parentElement;
                    while (ancestor && ancestor !== document.body) {
                        var computedTransform = getComputedStyle(ancestor).transform;
                        if (computedTransform && computedTransform !== 'none') { hasTransform = true; break; }
